...
variable "allowed_access_groups" {
  type        = map(map(string))
  description = "Map of Access group IDs to allow access. Leave empty to allow all groups"
  default     = {}
}
...
variable "additional_client_certs" {
  type        = string
  default     = 0
  description = "Extra client certificates"
}
