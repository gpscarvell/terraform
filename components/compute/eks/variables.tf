...
variable "helm_charts" {
  description = "Map of map of helm charts to create"
  type        = any
  default     = {}
}

variable "cert_manager_setup" {
  description = "Create cert-manager ClusterIssuer"
  type        = bool
  default     = false
}

variable "cert_manager_issuer_yaml" {
  description = "YAML file with ClusterIssuer definition"
  type        = string
  default     = ""
}
...
variable "loki_cert" {
  description = "Loki certs for Client Certificate Authentication (base64 encoded)"
  type = object({
    ca_crt     = string
    server_crt = string
    server_key = string
...
variable "prometheus_auth" {
  description = "Prometheus base auth"
  type = object({
    username = string
    password = string
  })
